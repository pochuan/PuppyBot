...
void PrMatrix::svd( PrMatrix& U, PrMatrix& D, PrMatrix& V ) const
{
  ...
  NEWMAT::Matrix         nm;
  NEWMAT::Matrix         nm_U;
  NEWMAT::DiagonalMatrix nm_D;
  NEWMAT::Matrix         nm_V;

  copyToNewmat( nm, flip );

  NEWMAT::SVD( nm, nm_D, nm_U, nm_V );
...
void PrMatrix::LUdecompSPD( PrMatrix& lu ) const
{
  ...
  for( int kk = 0; kk < m_col; kk++ )
  {
    ...
    lu( kk, kk ) = (Float) sqrt( elementAt( kk, kk ) - sum );
  }
}
...
// ===================================================================
void PrMatrix::copyToNewmat( NEWMAT::Matrix& dest, bool fTranspose ) const
{
  if( !fTranspose )
  {
    dest.ReSize( m_row, m_col );
...
void PrMatrix::copyToNewmat( NEWMAT::SymmetricMatrix& dest ) const
{
  dest.ReSize( m_row );
  for( int ii = 0; ii < m_row; ii++ )
  {
...
// upper-left corner is (1,1), not (0,0).
// ===================================================================
void PrMatrix::copyFromNewmat( NEWMAT::Matrix& src, bool fTranspose )
{
  if( !fTranspose )
  {
    for( int ii = 0; ii < m_row; ii++ )
    {
      for( int jj = 0; jj < m_col; jj++ )
      {
        elementAt( ii, jj ) = (Float) src( ii + 1, jj + 1 );
      }
    }
  }
  else
  {
    for( int ii = 0; ii < m_row; ii++ )
    {
      for( int jj = 0; jj < m_col; jj++ )
      {
        elementAt( ii, jj ) = (Float) src( jj + 1, ii + 1 );
      }
    }
  }
}

void PrMatrix::copyFromNewmat( NEWMAT::DiagonalMatrix& src )
{
  zero();
  for( int ii = 0; ii < m_row; ii++ )
  {
    elementAt( ii, ii ) = (Float) src( ii + 1, ii + 1 );
  }
}
#endif // PR_MISSING_NEWMAT10_LIB
